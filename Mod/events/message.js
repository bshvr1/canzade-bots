...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.channel.id == Config.Log.General_Chat) {
      var cannn =
        random[Math.floor(Math.random() * random.length)];
      let no = Math.floor(Math.random() * 130)
      if (no == 98) {
        message.channel.send("<@" + message.author.id + "> " + cannn + " ");
      }
    ...
    if(message.channel.id == Config.Log.General_Chat && !message.member.hasPermission('ADMINISTRATOR') && !message.member.hasPermission("VIEW_AUDIT_LOG")) return message.react(Config.Others.RedTick)
    ...
    let owners = [Config.Permissions.Sahip]
    ...
    this.client.channels.cache.get(Config.Log.Komut).send(new Discord.MessageEmbed().setColor("RANDOM").setTimestamp().setAuthor(message.author.tag, message.author.avatarURL()).setDescription(`${message.author.tag} - (${message.author.id}) komut kullandı "${cmd.help.name}" kullandığı kanal ${message.channel.name} - ${message.channel.id}`))
 }
};
